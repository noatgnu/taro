...
def build(query):
    root = etree.Element('Query', client="true", processor="json", limit="-1", header="1")
    doc = etree.ElementTree(root)
    datasetElement = etree.SubElement(root, "Dataset", name=query["dataset"], config=query["config"])
    for f in query["filters"]:
        filter = etree.SubElement(datasetElement, "Filter", name=f["name"], value=f["value"], filter_list="")
